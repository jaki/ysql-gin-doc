...
\item
  \textbf{posting list}: in postgres, a GIN index tuple maps key to \emph{posting
...
\textinline{word HASH, page ASC}. This is like the index at the back of a
book. I can easily ask what pages have the word \textinline{foo} using this
index:
...
\textinline{word HASH, pages ASC}, where \textinline{word} is a word in
\textinline{words}. If I insert a page with 300 unique words and 500 total
...
\begin{center}
    \begin{tabular}{lll}
        ...
        \sqlinline{jsonb_ops} & \sqlinline{jsonb} & \sqlinline{?}, \sqlinline{?&},
        \sqlinline{?|}, \sqlinline{@>}, \sqlinline{@?},
        \sqlinline{@@} \\
...
If I \sqlinline{CREATE INDEX ON bar USING gin (jsonb_col)}, I
implicitly use opclass \sqlinline{jsonb_ops}. To use
\sqlinline{jsonb_path_ops}, I need to
...
\sqlinline{jsonb_path_ops} forms an index key for each path from root to
leaf.
...
\begin{oparts}
\item
  set \textinline{a} contains element \textinline{b}? (\sqlinline{@>})
...
For example, the \sqlinline{=} operator for arrays makes sure that the
...
For postgres,
\sqlinline{INSERT INTO table_with_gin_index (to_tsvector('simple', 'the quick brown'))}
does
...
For postgres,
\sqlinline{SELECT * FROM table_with_gin_index WHERE tscol @@ to_tsquery('simple', 'the')}
does
...
\textinline{brown}. To support prefix search, serialization of the words
...
Contains (\sqlinline{@>}) searches can have corresponding scan
keys
...
\begin{oparts}
\item
  \sqlinline{25.0} and \sqlinline{25} match should equally match--what will
...
\item
  ...
  marking array end on array GIN index.) \cppinline{kGroupEnd}? But doesn't
...
\textinline{IndexScan} on
\sqlinline{c = 'b'} can look in this
index for \textinline{["b"]} prefix, get the next key component
\textinline{true}, then look up \textinline{true} in the indexed table.
...
\begin{nparts}
\item
  insert to main table (\cinline{ExecInsert},
  \cinline{table_tuple_insert})
\item
  insert to gin index (\cinline{ExecInsert},
  \cinline{ExecInsertIndexTuples})
