...
\item
  $\opget(I, f)$:
  given index $I$ and predicate $f$, return list of tuples $T = \braces{(\ik,
  \pk) \in I : f(\ik)}$.  $f$ should be a simple predicate like
  \begin{oparts}
  \item
    $\ik < c$
  \item
...
\item
  $\opand(T_1, T_2)$:
  given list of tuples $T_1$, $T_2$, return $T = T_1 \cap T_2$
\item
  $\opor(T_1, T_2)$:
  given list of tuples $T_1$, $T_2$, return $T = T_1 \cup T_2$
\item
  $\oprecheck(T_1, g)$:
  given list of tuples $T_1$ and predicate $g$, return list of tuples $T =
  \braces{(\ik, \pk) \in T_1 : g(\pk)}$.  $g$ may be any predicate like
  \begin{oparts}
  \item
...
\begin{nparts}
\item
  For each element in the query array, get a set of row ids whose array column
  \emph{contains} the element
\item
  Intersect the sets of row ids (\emph{logical and}) to obtain a set of row ids
  whose array column contains the query array (\sqlinline{@>})
\item
