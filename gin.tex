...
\item
  $\opget(I, f)$:
  given index $I$, return list of tuples $T = \braces{(\ik, \pk) \in I :
  f(\ik)}$ for $f$ a simple predicate like
  \begin{oparts}
  \item
    $\ik < c$
  \item
...
\item
  $\oprecheck(T_1, g)$:
  given list of tuples $T_1$, return list of tuples $T = \braces{(\ik, \pk) \in
  T_1 : g(\pk)}$ for $g$ any predicate like
  \begin{oparts}
  \item
